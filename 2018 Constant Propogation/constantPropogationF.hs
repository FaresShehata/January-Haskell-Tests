...
scan f ((Assign v e) : ss)
  | v == "$return" = (f', s':ss')
  | Const n <- e'  = (sub v n . f', ss')
  | otherwise      = (f', s':ss')
  where
    e' = f e
    s' = Assign v e'
...
scan f ((If e b1 b2) : ss)
  = (f1 . f2 . f', (If (f e) b1' b2') : ss')
  where
    (f1, b1') = scan f b1
    (f2, b2') = scan f b2
    (f', ss') = scan f ss

scan f ((DoWhile b e) : ss)
  = (fb' . f', (DoWhile b' (f e)) : ss')
  where
    (fb', b') = scan f b
    (f', ss') = scan f ss
