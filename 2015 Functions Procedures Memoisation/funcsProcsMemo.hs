...
updateVar (var, val) s
  | bindings == [] = (var, (Local, val)) : s
  | otherwise      = (var, (scope, val)) : rest ++ others
  where
    (bindings, others) = partition ((== var) . fst) s
    ((_, (scope, _)) : rest) = bindings 
...
applyOp Mul (I m) (I n)
  = I (m * n)
applyOp Less (I m) (I n)
  | m < n     = I 1
  | otherwise = I 0
applyOp Equal (I m) (I n)
  | m == n    = I 1
  | otherwise = I 0
...
eval (Var var) _ s
  = getValue var s
eval (OpApp op e1 e2) fds s
  = applyOp op (eval e1 fds s) (eval e2 fds s)
eval (Cond p e1 e2) fds s
  | eval p fds s == I 1 = eval e1 fds s
  | otherwise           = eval e2 fds s
eval (FunApp f args) fds s
  = eval e fds s'
  where
    (vars, e) = lookUp f fds
    vals = evalArgs args fds s
    s' = bindArgs vars vals ++ s
...
executeStatement (Assign var e) fds _ s
  = updateVar (var, eval e fds s) s

executeStatement (AssignA avar ei ev) fds _ s
  = updateVar (avar, assignArray a i v) s
  where
    a = getValue avar s
    [i, v] = map (\e -> eval e fds s) [ei, ev]

executeStatement (If p b1 b2) fds pds s
  | eval p fds s == I 1 = executeBlock b1 fds pds s
  | otherwise           = executeBlock b2 fds pds s

executeStatement w@(While p b) fds pds s
  | eval p fds s == I 0 = s
  | otherwise           = executeStatement w fds pds s'
  where
    s' = executeBlock b fds pds s

executeStatement (Call var prc args) fds pds s
  | var == "" = s'
  | otherwise = updateVar (var, res) s'
  where
    (vars, b) = lookUp prc pds
    vals = evalArgs args fds s
    argsState = bindArgs vars vals

    rs = executeBlock b fds pds (argsState ++ getGlobals s)
    res = getValue "$res" rs
    s' = getLocals s ++ getGlobals rs
...
-- executeBlock [] _ _ s = s
-- executeBlock (s:ss) fds pds state
--   = executeBlock ss fds pds (executeStatement s fds pds state)
executeBlock b fds pds state
  = foldl (\state' s -> executeStatement s fds pds state') state b 
...
hasMemoFun :: Exp -> [(Id, Id)] -> Bool
hasMemoFun (Const _) _ = False
hasMemoFun (Var _)   _ = False
hasMemoFun (OpApp _ e1 e2) fm
  = hasMemoFun e1 fm || hasMemoFun e2 fm
hasMemoFun (Cond p e1 e2) fm
  = hasMemoFun p fm || hasMemoFun e1 fm || hasMemoFun e2 fm
hasMemoFun (FunApp f _) fm
  = lookup f fm /= Nothing

-- expression, nameMap for functions, new variables
-- return (block, return expression, unused variables)
translate' :: Exp -> [(Id, Id)] -> [Id] -> (Block, Exp, [Id])
translate' e@(Const _) _ ids
  = ([], e, ids)

translate' e@(Var _) _ ids
  = ([], e, ids)

translate' e@(OpApp op e1 e2) fm ids
  | hasMemoFun e fm = (b1 ++ b2 ++ [Assign v (OpApp op e1' e2')], Var v, ids')
  | otherwise       = ([], e, ids)
  where
    (b1, e1', ids1) = translate' e1 fm ids
    (b2, e2', ids2) = translate' e2 fm ids1
    (v:ids') = ids2

translate' e@(Cond p e1 e2) fm ids
  | hasMemoFun e fm = (bp ++ [If p'
                                (b1 ++ [Assign v e1'])
                                (b2 ++ [Assign v e2'])], Var v, ids')
  | otherwise       = ([], e, ids)
  where
    (bp, p' , idsp) = translate' p  fm ids
    (b1, e1', ids1) = translate' e1 fm idsp
    (b2, e2', ids2) = translate' e2 fm ids1
    (v:ids') = ids2

translate' e@(FunApp f args) fm ids
  | mp /= Nothing = ([Call v (fromJust mp) args], Var v, ids')
  | otherwise     = ([], e, ids)
  where
    mp = lookup f fm
    (v:ids') = ids
